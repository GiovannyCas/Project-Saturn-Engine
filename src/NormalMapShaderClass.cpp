#include "../src/header/NormalMapShaderClass.h"
...
bool NormalMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"src/HLSL/normalMapVertexShader.hlsl");
	if (error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"src/HLSL/normalMapPixelShader.hlsl");
	if (error != 0)
	{
		return false;
	}
