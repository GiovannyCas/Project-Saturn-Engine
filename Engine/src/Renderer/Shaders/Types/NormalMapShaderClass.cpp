#include <header/NormalMapShaderClass.h>
...
bool NormalMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"Engine/src/HLSL/normalMapVertexShader.hlsl");
	if (error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"Engine/src/HLSL/normalMapPixelShader.hlsl");
	if (error != 0)
	{
		return false;
	}
...
bool NormalMapShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DirectX::XMMATRIX worldMatrix, DirectX::XMMATRIX viewMatrix, DirectX::XMMATRIX projectionMatrix,
	ID3D11ShaderResourceView* texture1, ID3D11ShaderResourceView* texture2, DirectX::XMFLOAT3 lightDirection, DirectX::XMFLOAT4 diffuseColor)
{
	bool result;
...
bool NormalMapShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DirectX::XMMATRIX worldMatrix, DirectX::XMMATRIX viewMatrix, DirectX::XMMATRIX projectionMatrix,
	ID3D11ShaderResourceView* texture1, ID3D11ShaderResourceView* texture2, DirectX::XMFLOAT3 lightDirection, DirectX::XMFLOAT4 diffuseColor)
{
	HRESULT result;
