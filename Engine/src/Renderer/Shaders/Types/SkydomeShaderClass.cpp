#include <header/SkydomeShaderClass.h>
...
bool SkyDomeShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"Engine/src/HLSL/skydomeVertexShader.hlsl");
	if (error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"Engine/src/HLSL/skydomePixelShader.hlsl");
	if (error != 0)
	{
		return false;
	}
